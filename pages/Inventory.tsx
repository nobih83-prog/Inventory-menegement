...
const Inventory: React.FC = () => {
  ...
  const openAddModal = () => {
    ...
    setFormData({ 
      name: '', 
      sku: generateSKU(initialCategory), 
  ...
  const handleCategoryChange = (cat: string) => {
    setFormData(prev => ({
      ...prev,
      category: cat,
      // Only auto-update SKU if we are in Add Mode
      sku: isEditMode ? prev.sku : generateSKU(cat)
    }));
  };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { font-family: 'Inter', sans-serif; width: 58mm; height: 40mm; margin: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 2mm; box-sizing: border-box; }
        ...
        <body>
          ...
          <div class="barcode-container"><svg id="${svgId}"></svg></div>
          <div class="sku">${item.sku}</div>
          <script>
            window.onload = function() {
              JsBarcode("#${svgId}", "${item.sku}", { format: "CODE128", width: 2, height: 40, displayValue: false, margin: 0 });
  ...
  const handleSaveItem = () => {
    if (isEditMode) {
      ...
      addNotification({ title: 'Item Added', message: `${formData.name} added to catalog with SKU ${formData.sku}.`, type: 'success' });
    }
  ...
  const handleStockAdjustment = () => {
    ...
    addNotification({ title: type === 'IN' ? 'Restocked' : 'Stock Removed', message: `${selectedItem.name} balance is now ${finalVal}.`, type: 'info' });
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredInventory.map((item) => (
                <tr key={item.id} onClick={() => { setSelectedItem(item); setIsDetailModalOpen(true); }} className={`text-sm text-slate-700 hover:bg-slate-50/80 transition-all cursor-pointer group ${activeHighlightId === item.id ? 'animate-highlight' : ''}`}>
                  <td className="px-6 py-4"><div className="flex flex-col"><span className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{item.name}</span><span className="text-[10px] font-mono text-slate-400">{item.sku}</span></div></td>
                  <td className="px-6 py-4 font-black">{currencySymbol}{item.price.toFixed(2)}</td>
                  <td className="px-6 py-4 text-center font-black"><span className={item.stock < item.minStockLevel ? 'text-rose-600' : 'text-slate-900'}>{item.stock}</span></td>
                  <td className="px-6 py-4"><div className="flex items-center"><div className={`w-2 h-2 rounded-full mr-2 ${item.stock < item.minStockLevel ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500'}`}></div><span className="text-[10px] font-black uppercase">{item.stock < item.minStockLevel ? 'Low' : 'OK'}</span></div></td>
                  <td className="px-6 py-4 text-right overflow-visible">
                    <div className="relative inline-block text-left" ref={activeMenuId === item.id ? menuRef : null}>
                      <button onClick={(e) => toggleMenu(e, item.id)} className={`p-2 rounded-xl transition-all ${activeMenuId === item.id ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-indigo-600'}`}><MoreVertical size={18} /></button>
                      {activeMenuId === item.id && (
                        <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-[100] animate-in zoom-in-95 origin-top-right">
                          <button onClick={(e) => { e.stopPropagation(); openEditModal(item); }} className="w-full flex items-center px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"><Edit size={14} className="mr-3 text-slate-400" /> Edit Item</button>
                          <button onClick={(e) => { e.stopPropagation(); handlePrintLabel(item); }} className="w-full flex items-center px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"><Barcode size={14} className="mr-3 text-indigo-500" /> Print Label</button>
                          <div className="h-px bg-slate-100 my-1 mx-2"></div>
                          <button onClick={(e) => { e.stopPropagation(); openRestockModal(item, 'IN'); }} className="w-full flex items-center px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"><ArrowUpCircle size={14} className="mr-3 text-emerald-500" /> Restock</button>
                          <button onClick={(e) => { e.stopPropagation(); openRestockModal(item, 'OUT'); }} className="w-full flex items-center px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"><ArrowDownCircle size={14} className="mr-3 text-amber-500" /> Remove Stock</button>
                          <div className="h-px bg-slate-100 my-1 mx-2"></div>
                          <button onClick={(e) => { e.stopPropagation(); openDeleteModal(item); }} className="w-full flex items-center px-4 py-3 text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors"><Trash2 size={14} className="mr-3 text-rose-400" /> Delete Item</button>
                        </div>
                      )}
                    </div>
                  </td>
      ...
      {isRestockModalOpen && selectedItem && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 text-center animate-in zoom-in-95">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6 ${selectedItem.adjustmentType === 'IN' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>{selectedItem.adjustmentType === 'IN' ? <ArrowUpCircle size={32} /> : <ArrowDownCircle size={32} />}</div>
            ...
            <div className="flex flex-col space-y-3">
              <button onClick={handleStockAdjustment} className={`w-full py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest text-white ${selectedItem.adjustmentType === 'IN' ? 'bg-emerald-600 shadow-emerald-100' : 'bg-amber-600 shadow-amber-100'} shadow-xl`}>Confirm Adjustment</button>
      ...
      {isDeleteModalOpen && selectedItem && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-10 text-center animate-in zoom-in-95">
            <div className="w-20 h-20 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mx-auto mb-6"><AlertTriangle size={40} className="animate-pulse" /></div>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  <select value={formData.category} onChange={e => handleCategoryChange(e.target.value)} className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold bg-white">
                    <option>Coffee</option><option>Dairy</option><option>Supplies</option><option>Bakery</option>
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">SKU Code</label>
                  <div className="relative">
                    <input value={formData.sku} onChange={e => setFormData({...formData, sku: e.target.value.toUpperCase()})} type="text" className="w-full pl-5 pr-12 py-3.5 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all text-sm font-bold uppercase" />
                    <button 
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input value={formData.stock} onChange={e => setFormData({...formData, stock: parseInt(e.target.value) || 0})} type="number" className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold" />
                </div>
                <div className="space-y-1.5">
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="px-8 py-7 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center space-x-5">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white ${selectedItem.stock < selectedItem.minStockLevel ? 'bg-rose-600' : 'bg-indigo-600'}`}><Package size={28} /></div>
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-3 gap-6">
                 <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Stock</p><span className={`text-4xl font-black ${selectedItem.stock < selectedItem.minStockLevel ? 'text-rose-600' : 'text-slate-900'}`}>{selectedItem.stock}</span></div>
                 <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Price</p><span className="text-3xl font-black">{currencySymbol}{selectedItem.price.toFixed(2)}</span></div>
                 <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Category</p><span className="text-sm font-black text-indigo-600">{selectedItem.category}</span></div>
              </div>
