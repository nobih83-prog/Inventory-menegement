...
const Customers: React.FC = () => {
  ...
  const executeDelete = () => {
    if (customerToDelete) {
      // Record who deleted the customer
                      ...
                      <div className="relative inline-block text-left" ref={activeMenuId === c.id ? menuRef : null}>
                        ...
                        {activeMenuId === c.id && (
                          <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-[100] animate-in zoom-in-95 duration-150 origin-top-right">
                            <button onClick={(e) => { e.stopPropagation(); setIsDetailModalOpen(true); setSelectedCustomer(c); setActiveMenuId(null); }} className="w-full flex items-center px-4 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); handleCall(e, c.phone); }} className="w-full flex items-center px-4 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                              <Phone size={14} className="mr-3 text-emerald-500" /> Voice Call
                            </button>
                            ...
                            <button onClick={(e) => e.stopPropagation()} className="w-full flex items-center px-4 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                              <Edit size={14} className="mr-3 text-slate-400" /> Edit Details
                            </button>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-12">
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between">
                  ...
                  <button className="text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest py-2.5 px-6 bg-indigo-50 rounded-2xl transition-all">Export Statement</button>
                </div>
